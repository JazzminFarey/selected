...
R=Path(__file__).resolve().parents[2]; A=R/'Assets'; T=Path('/tmp/director-reel');T.mkdir(exist_ok=True)
W,H,FPS=1280,720,30
...
# Seconds are frame exact. The final third prioritises Flex; no project roll-call.
shots=[('money-motion',0,2.4,'Money Your Way'),('flex-pair',2.4,3.2,'That’s A Flex'),('flex-open',3.2,4.8,'That’s A Flex'),('money-pair',4.8,6.3,'Money Your Way'),('creator',6.3,7.8,'Built for Feed'),('mugs',7.8,10,'allHearts'),('products',10,11,'allHearts'),('buddies',11,12,'Barnardos Buddies'),('posters',12,13.2,'Barnardos Buddies'),('jump',13.2,15.4,'Jump Rope'),('walk',15.4,17.4,'Walk Your Way'),('ooh',17.4,19.2,'That’s A Flex'),('money-motion-return',19.2,21,'Money Your Way'),('flex-grid',21,22.6,'That’s A Flex'),('ooh-retail',22.6,23.2,'That’s A Flex'),('money-single',23.2,23.7,'Money Your Way'),('ooh-yellow',23.7,24.2,'That’s A Flex'),('money-man',24.2,24.7,'Money Your Way'),('ooh-billboard',24.7,25.2,'That’s A Flex'),('flex-build',25.2,27.8,'That’s A Flex'),('flex-finish',27.8,30,'That’s A Flex')]
def put(c,im,box):
...
def draw(k,t,d):
 ...
 if k=='creator':
  ...
  for n,key in enumerate(['creator','creator2','travel']):put(c,key,(n*426,12*math.sin(math.pi*u+n),426,696))
  return c
...
draw('money-motion',1.1,2.4).save(A/'reel/poster.webp',quality=93)
